...
class Template
{
    ...
    public function output($scripts = array(), $styles = array(), $stime = '')
    {
        ...
        $inpageUrl = false;
        if (!$this->isAmp) {
            if ($this->cache && !($inpageUrl = $this->cache->get(
                    _CACHE_PREFIX_ . 'inpageurl|' . $this->LANGUAGE . '|' . $md5url
                ))) {
                ...
                $buffer = str_replace(
                    '</head>',
                    '<style>' . $this->cache->get($inpageUrl) . '</style></head>',
                    $buffer
                );
            }
        } else {
    ...
    public function load_template($scripts, $styles)
    {
        ...
        if (UserController::getCurrentUser()) {
            ...
            $this->template = str_replace(
                '<div class="user-panel"><div class="pull-left info"><p>{adminName}</p></div></div>',
                '',
                $this->template
            );
        ...
        foreach ($pagesArray as $page) {
            if ($_SERVER['REQUEST_URI'] == _FOLDER_URL_ . $langUrl . $page->url && !empty($page->metaog)) {
                ...
                /** @lang text */ '/(\<meta property\=\"og\:title\" content\=\")(.*)(\" \/\>)/',
                                  "$1{$metaog['title']}$3",
                                  $this->template,
                                  1
                );
                $this->template = preg_replace(
                /** @lang text */ '/(\<meta property\=\"og\:description\" content\=\")(.*)(\" \/\>)/',
                                  "$1{$metaog['description']}$3",
                                  $this->template,
                                  1
                );
                $replacementpic = '<meta property="og:image" content="http://' . _ADDRESS_ . '/img/' . $metaog['image'] . '" />';
                $this->template = preg_replace(
                /** @lang text */ '/\<meta property\=\"og\:image\" content\=\"(.*)\" \/\>/',
                                  $replacementpic,
                                  $this->template,
                                  1
                );
        ...
        */
        ...
        $menuR = (!$this->isAmp && arrayKeyExists('menu_right', $array_menu))?$this->menu(
            $array_menu['menu_right']
        ):'';
        if (!empty($menuR)) {
        ...
        if (!$this->isAmp) {
            $mainJavascripts = 'jquery.min.js,tether.min.js,bootstrap.min.js,jquery.easing.min.js,main.js';
            loadJs($mainJavascripts, $this->from_cache, false);
            $this->MAIN_JAVASCRIPTS = '<script src="' . _FOLDER_URL_ . 'cache/js/' . md5(
                    $mainJavascripts
                ) . '.js" id="mainjs" data-appdir="' . _FOLDER_URL_ . '"></script>';
        }
        ...
        if (count($this->css) > 0) {
            ...
            if (!empty($scripts)) {
                ...
                $cssLR = '<link rel="stylesheet" type="text/css" href="' . _FOLDER_URL_ . 'cache/css/' . md5(
                        $scripts
                    ) . '.css" id="cssdeferred" />';
            }
        }
                    ...
                    '<div class="col-12 col-lg-3 col-md-6 md-margin-50px-bottom sm-margin-30px-bottom last-paragraph-no-margin wow fadeInUp"' . ($index > 0?' data-wow-delay="0.' . ($index * 2) . 's"':'') . '>
                        <div class="blog-post blog-post-style1 text-center text-md-left">
                            ...
                            <div class="post-details">
                                <span class="post-author text-extra-small text-medium-gray text-uppercase d-block margin-10px-bottom sm-margin-5px-bottom">' . date(
                        'd M Y',
                        strtotime($post->date_updated)
                    ) . ' | de către Eurocom</span>
                ...
                if ($index < 3) {
                    $latestPostsFooter .= /** @lang text */
                        '<li class="media ' . ($index < 2?' border-bottom border-color-extra-light-gray':'') . '">
								...
								<div class="media-body text-small"><a href="' . $posthref . '">' . $post_content_footer . '</a> <span class="clearfix"></span>' . date(
                            'd M Y',
                            strtotime($post->date_updated)
                        ) . ' | de către Eurocom</div>
							</li>' . PHP_EOL;
                }
            }
                    ...
                    if (arrayKeyExists('id', $page) && arrayKeyExists($page['id'], $mArr)) {
                        ...
                        $menu .= '">
                                                ...
                                                <ul class="treeview-menu" style="display: none;">
                                                        <li><a href="' . $page['url'] . '">' . $hrefHtmlSub . '</a></li>' . $this->authMenu(
                                $mArr,
                                $page['id']
                            ) . '
                                                </ul>
                        ...
                        if ($page['url'] == '/help') {
                            $menu .= '<li><a href="#" data-toggle="modal" data-target="#wizard" data-backdrop="static" data-keyboard="false"><i class="fa fa-magic"></i> ' . _(
                                    'Wizard'
                                ) . '</a></li>';
                        }
                        $hrefHtml = $page['menu_text'];
                        ...
                        $menu .= '<li';
                        if ($_SERVER['REQUEST_URI'] == $page['url'] || strpos(
                                                                           $_SERVER['REQUEST_URI'],
                                                                           $page['url'] . '/'
                                                                       ) === 0) {
                            if (!arrayKeyExists('classes', $page)) {
                    ...
                    if (arrayKeyExists('id', $page) && arrayKeyExists($page['id'], $mArr)) {
                        ...
                        $menu .= '<li class="nav-item dropdown' . $ddcssClasses . '">
							...
							<ul class="dropdown-menu" aria-labelledby="menu' . $page['id'] . '">
								...
								<li><a class="dropdown-item' . $cssClasses . '" href="' . $page['url'] . '">' . $page['submenu_text'] . '</a>' . $this->menu(
                                $mArr,
                                $page['id']
                            ) . '</li>
							</ul>
