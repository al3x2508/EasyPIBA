        ...
        if (!$cache || empty($buffer)) {
            ...
            if (count($scripts) > 0) {
                foreach ($scripts as $script) {
                    switch ($script) {
                        case 'bootstrap.css':
                            $fileName = _APP_DIR_ . 'vendor/twbs/bootstrap/dist/css/bootstrap.min.css';
                            break;
                        case 'fontawesome.css':
                            $fileName = _APP_DIR_ . 'node_modules/@fortawesome/fontawesome-pro/css/all.min.css';
                            break;
                        default:
                            if (strpos($script, 'Module/') !== 0) {
...
if (arrayKeyExists('js', $_GET)) {
    ...
    if (in_array($page_url, ['main.css', 'css/main.css'])) {
        header("content-type: text/css");
        header('Cache-Control: public');
        header('Expires: ' . gmdate('D, d M Y H:i:s \G\M\T', time() + 2592000));
        $buffer = loadCss(
