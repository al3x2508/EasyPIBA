...
class Redis
{
    ...
    protected static $map = [
        ...
        // Lists
        'blpop' => [
            'vararg' => self::VAR_KEY_ALL_AND_TIMEOUT,
            'return' => 'Vector',
            'retargs' => [1]
        ],
        'brpop' => [
    ...
    {
        try {
            return $this->connect('127.0.0.1', self::DEFAULT_PORT, 5);
        } catch (\Exception $e) {
            return false;
        }
    }
    ...
    public function connect(
        ...
        return $this->doConnect($host, $port, $timeout, $persistent_id, $retry_interval, false);
    }
    ...
    protected function doConnect(
        ...
        if (!$this->connection) {
            trigger_error("Failed connecting to redis server at {$host}: {$errstr}", E_WARNING);
            return false;
        }
    ...
    public function setOption($opt, $value)
    {
        switch ($opt) {
            ...
            case self::OPT_SERIALIZER:
                if (($value !== self::SERIALIZER_NONE) && ($value !== self::SERIALIZER_PHP)) {
            ...
            case self::OPT_READ_TIMEOUT:
                ...
                $this->timeout_useconds = (int)(($value - $this->timeout_seconds) * 1000000);
                return stream_set_timeout($this->connection, $this->timeout_seconds, $this->timeout_useconds);
    ...
    public function pconnect(
        ...
        return $this->doConnect($host, $port, $timeout, $persistent_id, $retry_interval, true);
    }
    ...
    protected function checkConnection($auto_reconnect = true)
    {
        ...
        if ($auto_reconnect AND $this->doConnect($this->host, $this->port, $this->timeout_connect, null,
                $this->retry_interval, $this->persistent)
        ) {
            if ($this->password) {
    ...
    protected function sockReadData(&$type)
    {
        ...
        if (strlen($line)) {
            ...
            switch ($type) {
                ...
                default:
                    throw new RedisException("protocol error, got '{$type}' " . "as reply type byte");
            }
        }
        return null;
    }
    ...
    protected function processInfoResponse()
    {
        ...
        foreach ($lines as $line) {
            if ((substr($line, 0, 1) == '#') OR !trim($line)) {
                continue;
            }
            $colon = strpos($line, ':');
            if ($colon === false) {
    ...
    {
        $this->processCommand("INCRBYFLOAT", $this->_prefix($key), (float)$by);
    ...
    protected function processDoubleResponse()
    {
        if ($this->mode === self::ATOMIC) {
            $resp = $this->sockReadData($type);
            if (($type === self::TYPE_INT) || ($type === self::TYPE_BULK && is_numeric($resp))) {
                return (float)$resp;
            }
            return false;
        }
    ...
    public function set($key, $value, $optionArrayOrExpiration = -1)
    {
        ...
        if (is_array($optionArrayOrExpiration) && count($optionArrayOrExpiration) > 0) {
            ...
            if (is_numeric($optionArrayOrExpiration) && (int)$optionArrayOrExpiration > 0) {
    ...
    protected function processStringResponse()
    {
        if ($this->mode === self::ATOMIC) {
            $resp = $this->sockReadData($type);
            return (($type === self::TYPE_LINE) OR ($type === self::TYPE_BULK))?((string)$resp):null;
        }
    ...
    protected function zInterUnionStore(
        ...
        if ($weights) {
            ...
            foreach ($weights as $weight) {
                if (is_int($weight) OR is_float($weight) OR ($weight === 'inf') OR ($weight === '-inf') OR ($weight === '+inf')) {
                    $args[] = $weight;
                }
            }
        }
    ...
    {
        return $this->zRangeByScoreImpl('ZRANGEBYSCORE', $key, $start, $end, $opts);
    }
    ...
    protected function zRangeByScoreImpl(
        ...
        if (isset($opts['limit']) AND is_array($opts['limit']) AND (count($opts['limit']) == 2)) {
    ...
    {
        return $this->zRangeByScoreImpl('ZREVRANGEBYSCORE', $key, $start, $end, $opts);
    }
    ...
    protected function scanImpl($cmd, $key, &$cursor, $pattern, $count)
    {
        ...
        do {
            ...
            if (!is_array($resp) || count($resp) != 2 || !is_array($resp[1])) {
                throw new RedisException(sprintf("Invalid %s response: %s", $cmd, print_r($resp, true)));
            }
            $cursor = (int)$resp[0];
            $results = $resp[1];
    ...
    {
        throw new RedisException('Named persistent connections are ' . 'not supported.');
    }
    ...
    public function __call($fname, $args)
    {
        ...
        if (empty($func['handler'])) {
            $func['handler'] = empty($func['return'])?null:"process{$func['return']}Response";
        }
        ...
        for ($i = 0; $i < $flen; $i++) {
            if (!arrayKeyExists($i, $args)) {
                if (isset($func['defaults']) AND arrayKeyExists($i, $func['defaults'])) {
                    $args[$i] = $func['defaults'][$i];
                } else {
                    trigger_error("Redis::$fname requires at least $flen parameters $argc given", E_ERROR);
                    return null;
                }
            }
            switch ($format[$i]) {
                case 'k':
                    $args[$i] = $this->_prefix($args[$i]);
                    break;
                case 'v':
                    $args[$i] = $this->_serialize($args[$i]);
                    break;
                case 's':
                ...
                case 'p':
                    if (($args[$i] !== self::BEFORE) AND ($args[$i] !== self::AFTER)) {
                        trigger_error("Argument $i to Redis::$fname must be 'before' or 'after'", E_ERROR);
                        return null;
                    }
                    break;
            }
    ...
    protected function translateVarArgs(array $args, $flags)
    {
        ...
        if (($flags & self::VAR_TIMEOUT) AND (count($args) == 2) AND (is_array($args[0])) AND (is_int($args[1]))) {
            $args = $args[0] + [$args[1]];
        }
        if ((!($flags & self::VAR_TIMEOUT)) AND (count($args) == 1) AND (is_array($args[0]))) {
            $args = $args[0];
        }
        ...
        if (($this->prefix AND ($flags & self::VAR_KEY_MASK)) OR ($flags & self::VAR_SERIALIZE)) {
            $first = true;
            $varkey = $flags & self::VAR_KEY_MASK;
            foreach ($args as &$arg) {
                if (($first AND ($varkey == self::VAR_KEY_FIRST)) OR (!$first AND ($varkey == self::VAR_KEY_NOT_FIRST)) OR ($varkey == self::VAR_KEY_ALL)) {
                    $arg = $this->_prefix($arg);
                } else {
                    if ($flags & self::VAR_SERIALIZE) {
    ...
    protected function processSerializedResponse()
    {
        if ($this->mode === self::ATOMIC) {
            ...
            return (($type === self::TYPE_LINE) OR ($type === self::TYPE_BULK))?$this->_unserialize($resp):false;
        }
...
class RedisSessionModule implements \SessionHandlerInterface
{
    ...
    public function open($save_path, $session_name)
    {
        ...
        foreach ($paths as $path) {
            ...
            if ($args['weight'] < 1) {
                error_log("Invalid weight specified for session.save_path(redis), " . "assuming 1");
                $args['weight'] = 1;
            }
            ...
            if (isset($url['scheme']) && ($url['scheme'] == 'file')) {
                ...
                $args['host'] = $url['host'];
                $args['port'] = !empty($url['port'])?$url['port']:Redis::DEFAULT_PORT;
            }
    ...
    protected function connect($key)
    {
        ...
        if (($r['auth'] !== '') && !$redis->auth($r['auth'])) {
            return false;
        }
        if (($r['database'] !== 0) && !$redis->select($r['database'])) {
            return false;
        }
    ...
    protected function &selectWeight($key)
    {
        ...
        throw new RedisException("Ran out of weights selecting redis host " . "for session: $key");
    }
