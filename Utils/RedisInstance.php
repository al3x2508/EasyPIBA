...
class RedisInstance
{
    ...
    public function __construct()
    {
        if (class_exists('Redis')) {
            try {
                return $this->connect(_REDIS_HOST_, _REDIS_PORT_, 5);
            } catch (Exception $e) {
                return false;
            }
        }
    ...
    protected function checkConnection($auto_reconnect = true)
    {
        ...
        if ($auto_reconnect and $this->doConnect(
                $this->host,
                $this->port,
    ...
    protected function zInterUnionStore(
        ...
        if ($weights) {
            ...
            foreach ($weights as $weight) {
                if (is_int($weight) or is_float(
                        $weight
                    ) or ($weight === 'inf') or ($weight === '-inf') or ($weight === '+inf')) {
                    $args[] = $weight;
                }
            }
        }
