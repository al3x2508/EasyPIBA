...
namespace Model {
    ...
    class Model
    {
         ...
         */
        public function __construct(string $tableName, $schema = false)
        {
            //Connect to database
        ...
        private function getSchema()
        {
            ...
            $this->schema = $schema;
            if ($cache) {
                $cache->set(_CACHE_PREFIX_ . 'schema' . $this->tableName, json_encode($schema));
            }
        }
        ...
        public function create()
        {
            ...
            foreach (get_object_vars($this) as $key => $val) {
                if (!in_array($key, self::getIgnoredKeys()) && !is_object($val) && property_exists(
                        $this,
                        'schema'
         ...
         * @return array
         */
        private static function getIgnoredKeys(): array
        {
            return array(
                'db',
                'order_by',
        ...
        public function checkFields($method = 'insert')
        {
            ...
            foreach ($this->schema as $key => $param) {
                if (property_exists($this, $key)) {
                    switch ($param['param_type']) {
                        case 's':
                            ...
                            break;
                        case 'i':
                            if (empty($this->$key) && $param['null'] == 'NO' && filter_var(
                                    $this->$key,
                                    FILTER_VALIDATE_INT
                                ) === false) {
                ...
                } elseif ($method == 'insert' && $key != 'id' && $param['null'] == 'NO') {
                    $errors[$key] = 'notNull';
                }
            }
        ...
        public function getOneResult($key, $val)
        {
            ...
            $columnsToSelect = '';
            $joins = '';
            $this->add_auto_joins($join);
            $this->build_join_columns($join, $columnsToSelect, $joins);
            ...
            try {
                ...
                if ($stmt) {
                    ...
                    debug('Error preparing sql -> ' . $this->db->error . ' | ' . $key . ' = ' . $val);
                    return false;
                }
            } catch (Exception $e) {
                debug("Error sql: >> $sql << >> $val << {$e->getMessage()}");
         ...
         * @return Model[]|void
         */
        public function get(string $whereOp = 'AND', bool $ignore_password = true, bool $returnAsArray = false)
        {
            $where = '';
            //Array of mysql bind parameters; first element is the column types, the next ones are the values for the columns
            $data = array();
            ...
            $this->build_where($whereOp, $where, $param_type, $data, $join);
            ...
            try {
                ...
                debug("Error sql: >> $sql << >> " . join($data, " | ") . " << {$e->getMessage()}");
                return array('error' => $e->getMessage());
            }
            return $ret;
        }
         ...
         * @return int
         */
        public function countItems(string $whereOp = 'AND'): int
        {
            $totalItems = 0;
            $where = '';
            $data = array();
            ...
            if (!empty($this->customFields)) {
                foreach ($this->customFields as $customField) {
                    if (!is_array($customField)) {
                        $customFields .= (empty($customFields)) ? $customField : ', ' . $customField;
                    } else {
            ...
            foreach ($join as $table_reference => $column_reference) {
                $refColumn = (strpos(
                        ...
                        '.'
                    ) === false) ? $this->tableName . '.' . $column_reference['column'] : $column_reference['column'];
            ...
            if (property_exists($this, 'id')) {
                ...
                foreach (get_object_vars($this) as $key => $val) {
                    if (!is_a($val, 'Model\Model') && !in_array($key, self::getIgnoredKeys())) {
                        $param_type .= (property_exists($this, 'schema') && arrayKeyExists(
                                $key,
                                $this->schema
                            )) ? $this->schema[$key]['param_type'] : (is_numeric($val) ? 'i' : 's');
                    }
                }
                $param_type .= 'i';
                $data[] = &$param_type;
                ...
                if (count($this->where) > 0) {
                    foreach ($this->where as $key => $val) {
                        $w = (is_array(
                            $val
                        )) ? (($val[1] != 'BETWEEN') ? $key . ' ' . $val[1] . ' ?' : $key . ' BETWEEN ? AND ?') : $key . ' = ?';
                        $sql .= ' AND ' . $w;
                        $tipData = (is_numeric($val) ? 'i' : ((is_array($val) && arrayKeyExists(
                                2,
                                $val
                            )) ? $val[2] : 's'));
        ...
        public function delete($whereOp = 'AND')
        {
            ...
            $param_type = '';
            //Build the sql where for table columns and their parameter types
            $this->build_where($whereOp, $where, $param_type, $data);
            foreach (get_object_vars($this) as $key => $val) {
                if (!is_a($val, 'Model\Model') && !in_array($key, self::getIgnoredKeys())) {
                    if (is_array($val)) {
                        if ($val[1] != 'BETWEEN') {
        ...
        {
            $stmt = $this->db->query(/** @lang text */ "SELECT MAX(`id`) AS `id` FROM {$this->tableName}");
        ...
        public function runMultiQuery($sql)
        {
            try {
                ...
                debug("Error sql: >> $sql << {$e->getMessage()}");
                return false;
            }
        }

        private function add_auto_joins(&$join)
        {
                ...
                foreach ($this->schema as $column => $colDetails) {
                    if (!empty($colDetails['column_reference'])) {
                        $join[$colDetails['table_reference']] = array(
                            ...
                            'joinType' => (strtolower($colDetails['null']) == 'yes') ? 'LEFT' : 'INNER'
                        );
                    }
                }
            }
        ...
        private function build_join_columns($join, &$columnsToSelect, &$joins)
        {
            if (count($join)) {
                foreach ($join as $table_reference => $arrJoin) {
                    ...
                    $refColumn = (strpos(
                            $arrJoin['column'],
                            '.'
                        ) === false) ? $this->tableName . '.' . $arrJoin['column'] : $arrJoin['column'];
        ...
        private function build_where($whereOp, &$where, &$param_type, &$data, $join = []) {
            foreach (get_object_vars($this) as $key => $val) {
                if (!in_array($key, self::getIgnoredKeys())) {
                    $key = (count($join) > 0 && preg_match(
                            '/^([a-zA-Z0-9_\-]+)$/',
                            $key
                        )) ? $this->tableName . '.' . $key : $key;
                    $w = (is_array(
                        $val
                    )) ? (($val[1] != 'BETWEEN') ? $key . ' ' . $val[1] . ' ?' : $key . ' BETWEEN ? AND ?') : ($val !== null ? $key . ' = ?' : $key . ' IS NULL');
                    $where .= ($where == '') ? ' WHERE ' . $w : ' ' . $whereOp . ' ' . $w;
                    if ($val !== null) {
                        $tipData = (property_exists($this, 'schema') && arrayKeyExists(
                                $key,
                                $this->schema
                            )) ? $this->schema[$key]['param_type'] : (is_numeric($val) ? 'i' : 's');
                        $param_type .= ($key == 'id') ? 'i' : ((!is_array(
                                $val
                            ) || $val[1] != 'BETWEEN') ? $tipData : $tipData . $tipData);
                    }
                }
            }
