    ...
    class Model
    {
        ...
        private function getSchema(): void
        {
            ...
            $sql = sprintf(/** @lang text */ "SELECT `COLUMNS`.`COLUMN_NAME` AS `column`, `COLUMN_DEFAULT` AS `default`, `IS_NULLABLE` AS `null`, `DATA_TYPE`, `EXTRA` AS `extra`, `REFERENCED_TABLE_NAME` AS `table_reference`, `REFERENCED_COLUMN_NAME` AS `column_reference`, (SELECT GROUP_CONCAT(`COLUMN_NAME` SEPARATOR ',') FROM `INFORMATION_SCHEMA`.`COLUMNS` `c` WHERE `c`.`TABLE_SCHEMA` LIKE '%s' AND `TABLE_NAME` = `REFERENCED_TABLE_NAME`) AS `trc` FROM `INFORMATION_SCHEMA`.`COLUMNS` LEFT OUTER JOIN `INFORMATION_SCHEMA`.`KEY_COLUMN_USAGE` ON `KEY_COLUMN_USAGE`.`TABLE_SCHEMA` = `COLUMNS`.`TABLE_SCHEMA` AND `KEY_COLUMN_USAGE`.`TABLE_NAME` = `COLUMNS`.`TABLE_NAME` AND `KEY_COLUMN_USAGE`.`COLUMN_NAME` = `COLUMNS`.`COLUMN_NAME` WHERE `COLUMNS`.`TABLE_SCHEMA` LIKE '%s' AND `COLUMNS`.`TABLE_NAME` LIKE '%s'",
                _DB_NAME_, _DB_NAME_, $this->tableName);
        ...
        public function create()
        {
            ...
            foreach (get_object_vars($this) as $key => $val) {
                if (!in_array($key, self::getIgnoredKeys()) && !is_object($val) && property_exists($this,
                        'schema') && arrayKeyExists($key, $this->schema)) {
                    $param_type .= (property_exists($this, 'schema') && arrayKeyExists($key,
                            $this->schema))?$this->schema[$key]['param_type']:(is_numeric($val)?'i':'s');
                }
            }
            $data[] = &$param_type;
            $cols = array();
        ...
        public function get($whereOp = 'AND', $ignore_password = false, $returnAsArray = false)
        {
            ...
            if (count($join)) {
                foreach ($join as $table_reference => $arrJoin) {
                    foreach ($arrJoin['columns'] as $column) {
                        if(empty($this->group_by) || !empty($columnsToSelect)) $columnsToSelect .= ", ";
            ...
            foreach (get_object_vars($this) as $key => $val) {
                if (!in_array($key, self::getIgnoredKeys())) {
                    $key = (count($join) > 0 && preg_match('/^([a-zA-Z0-9_\-]+)$/',
                    ...
                    if ($val !== null) {
                        $tipData = (property_exists($this, 'schema') && arrayKeyExists($key,
                                $this->schema))?$this->schema[$key]['param_type']:(is_numeric($val)?'i':'s');
            ...
            if(!empty($this->group_by)) $tableAllColumns = '';
            $sql = "SELECT {$tableAllColumns} {$columnsToSelect} {$customFields} FROM {$this->tableName} {$joins} {$where} {$this->group_by} {$this->order_by} {$this->limit}";
            $ret = array();
            try {
                $stmt = $this->db->prepare($sql);
                if ($stmt) {
        ...
        public function getOneResult($key, $val)
        {
            ...
            try {
                ...
                if ($stmt) {
                    ...
                    trigger_error('Error preparing sql -> ' . $this->db->error . ' | ' . $key . ' = ' . $val);
                    return false;
                }
            } catch (Exception $e) {
                ...
                if (property_exists($this, $key)) {
                    switch ($param['param_type']) {
                        case 's':
                            ...
                            break;
                        case 'i':
                            if (empty($this->$key) && $param['null'] == 'NO' && filter_var($this->$key,
                    ...
                    if ($method == 'insert' && $key != 'id' && $param['null'] == 'NO') {
                        $errors[$key] = 'notNull';
                    }
                }
            }
