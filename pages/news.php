...
$content = '';
if($page_url == 'news' || preg_match('/news\/pag\-[\d+]/', $page_url)) {
	$pageno = preg_match('/news\/pag\-(\d+)/', $page_url, $matches)?$matches[1]:1;
	$limit = (($pageno - 1) * 6) . ', 6';
	$content = /** @lang text */
	...
	foreach($news AS $index => $story) {
		...
		$href = '/news/' . \Utils\Util::getUrlFromString($story->title) . '.html';
	...
	$title = str_replace(array('news/', '.html'), '', $page_url);
