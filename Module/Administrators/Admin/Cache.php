...
class Cache
{
    public function __construct()
    {
        ...
        if (arrayKeyExists('reread', $_REQUEST)) {
            ...
            $this->content .= '<div class="alert alert-success" role="alert"><strong>' . __(
                    'Modules reread'
                ) . '.</strong></div>';
            ...
            $this->content .= '<div class="alert alert-success" role="alert"><strong>' . __(
                    'Remove JS and CSS from cache dir'
                ) . '.</strong></div>';
...
//            $this->content .= '<div class="alert alert-success" role="alert"><strong>' . __('Clear APC cache ') . '.</strong></div>';
        ...
        $this->content .= '<form action="#" method="post"><input type="hidden" name="reread" value="1" /><input type="submit" class="btn btn-outline-primary" value="' . __(
                'Reread modules'
            ) . '" /></form>';
    }
