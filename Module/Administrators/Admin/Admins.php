...
<div class="card">
	<div class="card-header">
		<div class="row">
			<div class="col-md-9"><h3 class="card-title">' . __('Edit administrators') . '</h3></div>
			<div class="col-md-3">
			...
			<div class="col-12">
				<table id="data_table" class="table table-borderless table-hover table-sm w-100">
					<thead class="thead-light">
						<tr><th data-filtertype="text" data-filterid="idf">#</th><th data-filtertype="text" data-filterid="namef">' . __(
                'Name'
            ) . '</th><th data-filtertype="text" data-filterid="usernamef">' . __(
                'Username'
            ) . '</th><th data-filtertype="select" data-filterid="statusf" data-options=\'' . json_encode(
                $statusOptions,
                JSON_FORCE_OBJECT
            ) . '\'>' . __('Status') . '</th><th>' . __('Actions') . '</th></tr>
					</thead>
					<tbody>
					</tbody>
				</table>
			...
			<div class="modal-body" id="edtable">
			    <div class="form-group">
					<div class="input-group">
						<select class="form-control" id="edbranch" name="edbranch">' . Util::arrayToOptions(
                $branchOptions
            ) . '</select>
						<label for="edbranch" class="control-label">' . __('Branch') . '</label>
						<i class="bar"></i>
					</div>
                </div>
                <div class="form-group">
					<div class="input-group">
						<select class="form-control" id="eddepartment" name="eddepartment" required>' . Util::arrayToOptions(
                $departmentOptions
            ) . '</select>
						<label for="eddepartment" class="control-label">' . __('Department') . '</label>
						<i class="bar"></i>
					</div>
                </div>
				<div class="form-group">
                ...
                <div class="form-group">
					<div class="input-group">
						<select id="edstatus" name="edstatus" class="form-control"><option value="1">' . __(
                'Active'
            ) . '</option><option value="2">' . __('Blocked') . '</option></select>
