...
class EmailConfirmPage
{
    ...
    public function __construct()
    {
        ...
        $showForm = false;
        $codeError = ($code && strlen($code) != 32) ? '<div class="alert alert-danger">' . __(
        ...
        $form = '<form action="#" method="post">
			...
			<div class="col-12 mt-5 field form-group">
				<div class="input input-hoshi">
					<input type="text" name="code" id="code" class="input__field input__field-hoshi form-control" data-rule="maxlen:32" data-msg="' . sprintf(
                __('Enter at least %s characters'),
                '32'
            ) . '" pattern=".{32,32}" required />
					<label class="input__label input__label-hoshi input__label-hoshi-color-1" for="code" data-ex="' . __(
                '32 characters code'
            ) . '">
        ...
        if (!empty($code)) {
			...
			' . sprintf(
                            __('You can login now %s'),
                            '<a href="' . _ADDRESS_ . _FOLDER_URL_ . 'login">' . __('here') . '</a>'
                        ) . PHP_EOL;
                } else {
                    $this->content .= '<h1> ' . __('Your account is blocked!') . '!</h1>' . PHP_EOL;
                }
            } else {
                $showForm = true;
                $form = str_replace(
                    ...
                    '<div class="alert alert-danger">' . __(
                        'Code is invalid or already confirmed'
                    ) . '</div>',
                    $form
                );
            }
