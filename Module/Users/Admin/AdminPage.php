...
class AdminPage extends \Controller\AdminPage
{
    public function output()
    {
        ...
        foreach ($countries as $country) {
            $countriesOptions[$country->id] = ($country->code != 'RO')?$country->name:[$country->name, 'selected'];
        }
			...
			<div class="col-12">
				<table id="data_table" class="table table-borderless table-hover table-sm w-100">
					<thead class="thead-light">
						<tr><th data-filtertype="text" data-filterid="idf">#</th><th data-filtertype="text" data-filterid="firstnamef">' . __(
                'Firstname'
            ) . '</th><th data-filtertype="text" data-filterid="lastnamef">' . __(
                'Lastname'
            ) . '</th><th data-filtertype="select" data-filterid="countryf" data-options=\'' . json_encode(
                $countriesOptions,
                JSON_FORCE_OBJECT
            ) . '\'>' . __('Country') . '</th><th data-filtertype="text" data-filterid="emailf">' . __(
                'Email'
	...
	<div class="card-footer">
		<div class="btn-toolbar">
			...
			<button class="btn btn-secondary btn-export btn-sm" data-type="excel"><i class="fa fa-file-excel"></i> ' . __(
                'Export to Excel'
            ) . '</button>
			<button class="btn btn-secondary btn-export btn-sm" data-type="pdf"><i class="fa fa-file-pdf"></i> ' . __(
                'Export to PDF'
            ) . '</button>
		</div>
	</div>
