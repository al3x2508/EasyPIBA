...
class LoginPage
{
    ...
    public function __construct()
    {
        ...
        $register_errors = '';
        //Check login
        if (arrayKeyExists('email', $_POST) && arrayKeyExists('password', $_POST) && !arrayKeyExists(
                'confirmPassword',
                $_POST
            ) && arrayKeyExists('CSRFName', $_POST) && arrayKeyExists('CSRFToken', $_POST)
        ) {
            if (Util::csrfguard_validate_token($_POST['CSRFName'], $_POST['CSRFToken'])) {
                ...
                if ($user) {
                    ...
                    if ($status != 1) {
                        ...
                        $login_errors[] = ($status == 0) ? __('You did not confirmed your account') : __(
        ...
        $this->content .= '						<div class="col-12 mt-5 field form-group">
											<div class="input-group">
												<input type="email" name="email" id="email-login" class="form-control"' . $emailValue . ' data-rule="maxlen:2" data-msg="' . __(
                'Enter your email'
            ) . '" pattern="^(?:[\w\d-]+.?)+@(?:(?:[\w\d]-?)+.)+\w{2,4}$" required />
										...
										<div class="col-12 mt-5 field form-group">
                                            <div class="custom-control custom-checkbox">
                                                ...
                                                <label for="remember" class="custom-control-label"> ' . __(
                'Remember me'
            ) . '</label>
                                            </div>
                                        </div>
                ...
                'Log in'
										...
										<div class="col-12 field form-group">
											<a href="' . _ADDRESS_ . _FOLDER_URL_ . 'password_reset" class="forgot-password">' . __(
                'Forgot Password?'
            ) . '</a> | 
											<a href="#" class="forgot-password" id="resend">' . __(
                'Resend email confirmation'
            ) . '</a>
										</div>
									</form>';
        $this->content .= Controller::getAccountForm(
                $fields,
                array(),
                'register-form',
                __('Register'),
