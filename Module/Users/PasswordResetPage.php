...
class PasswordResetPage
{
    ...
    public function __construct()
    {
        ...
        if (arrayKeyExists('email', $_POST)) {
            ...
            if (!Util::checkFieldValue('email', $email)) {
                $fields['email'] = __('Enter a valid email address');
            } else {
				...
				<p><a href="' . $resetLink . '">' . $resetLink . '</a></p>';
                    ...
                    $this->content = '<div class="col-12"><h3>' . __(
                            'An email with password reset instructions have been sent to you.'
                        ) . '</h3></div>';
                } else {
                    $fields['email'] = __('No user registered with this email address');
                }
            }
            ...
            if (arrayKeyExists('password', $_POST) && count($fields) == 0) {
                ...
                if (count($resetModel)) {
                    ...
                    $user->password = $bcrypt->hash(
                        strip_tags(htmlspecialchars(stripslashes(trim($_POST['password']))))
                    );
                    Controller::logUserActivity('Password reseted');
                    $user->update();
                    $this->content = '<div class="col-12"><h2>' . __('Your password has been updated') . '</h2></div>';
                } else {
                    $errors = '<div class="alert alert-danger">' . __('Code not found') . '</div>';
                }
            } else {
			...
			<div class="col-12 col-lg-6 col-lg-offset-3 col-md-6 col-md-offset-3">
				...
				<div class="panel panel-login">
					<div class="panel-heading container"><div class="row"><div class="col-12 text-center"><h2><b>' . __(
						...
						<div class="row">
							<div class="col-12">
								<form action="' . _ADDRESS_ . _FOLDER_URL_ . 'password_reset" method="post" autocomplete="off" class="validateform">
									<input type="hidden" name="code" value="' . strip_tags(
                        htmlspecialchars(stripslashes(trim($_REQUEST['code'])))
                    ) . '" />
        ...
        if (empty($this->content)) {
            $email = array(
                'value' => (arrayKeyExists('email', $_POST)) ? ' value="' . strip_tags(
                        htmlspecialchars(stripslashes(trim($_POST['email'])))
                    ) . '"' : '',
                'validation' => (arrayKeyExists(
                    'email',
                    $fields
                )) ? '<div class="alert alert-danger">' . $fields['email'] . '</div>' : ''
            );
            $this->content = '<div class="row justify-content-md-center my-4">
				<div class="col-12 col-lg-6 col-lg-offset-3 col-md-6 col-md-offset-3">
					' . $errors . '
					<div class="panel panel-login">
						<div class="panel-heading"><div class="row"><div class="col-12 text-center"><h2><b>' . __(
                    'Password reset'
                ) . '</b></h2></div></div><hr /></div>
                    ...
                    'Enter your email'
                ) . '"' . $email['value'] . ' pattern="^(?:[\w\d-]+.?)+@(?:(?:[\w\d]-?)+.)+\w{2,4}$" required />
