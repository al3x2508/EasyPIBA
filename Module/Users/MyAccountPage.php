...
class MyAccountPage
{
    ...
    public function __construct()
    {
        ...
        if ($user) {
            ...
            $this->myaccountlink = '<a href="' . _FOLDER_URL_ . 'my-account" class="btn btn-sm btn-default btn-flat">' . __(
                    'My account'
                ) . '</a>';
            ...
            if (arrayKeyExists('firstname', $_POST)) {
                ...
                if (count($fields) == 0) {
                    ...
                    if (empty($message)) {
                        if ($lastname = @ucwords(
                            strtolower(strip_tags(htmlspecialchars(stripslashes(trim($_POST['lastname']))))),
                            " -"
                        )
                        ) {
                            $firstname = ucwords(
                                strtolower(strip_tags(htmlspecialchars(stripslashes(trim($_POST['firstname']))))),
                                " -"
                            );
                        } else {
                            $lastname = Util::ucname(
                                strip_tags(htmlspecialchars(stripslashes(trim($_POST['lastname']))))
                            );
                            $firstname = Util::ucname(
                                strip_tags(htmlspecialchars(stripslashes(trim($_POST['firstname']))))
                            );
                        }
                ...
                if (arrayKeyExists('confirmPassword', $_POST) && !Util::checkFieldValue(
                        'confirmPassword',
            ...
            $this->content .= '<div class="row justify-content-md-center my-4">
				<div class="col-12 col-lg-6 col-lg-offset-3 col-md-6 col-md-offset-3">
					<div class="panel panel-login">
						<div class="panel-heading container"><div class="row"><div class="col-12 text-center"><h2><b><i class="fa fa-user"></i> ' . __(
                    'My account'
                ) . '</b></h2></div></div><hr /></div>
						<div class="panel-body container">
							<div class="row">
								<div class="col-12">' . PHP_EOL . $alert . '
									...
									<!-- End viewinfo -->
									<!-- Start editinfo -->
									<div id="editinfo"' . ((empty($message) || arrayKeyExists(
                        'password',
                        $_POST
                    )) ? ' style="display:none;"' : '') . '>
										' . Controller::getAccountForm($fields, $values, 'myaccount', false) . '
									</div>
									<div id="editpasswordF"' . ((empty($message) || !arrayKeyExists(
                        'password',
                        $_POST
                    )) ? ' style="display:none;"' : '') . '>
                    ...
                    '8'
													...
													<label class="control-label" for="password" data-ex="' . __(
                    ...
                    '8 characters minimum'
								...
								<div class="col-12 mb-3">
									<div class="d-flex justify-content-sm-center">
										<div class="col-sm-6">
											<button id="edit" class="form-control btn btn-outline-primary" data-action="edit">' . __(
                    'Edit'
                ) . '</button>
										</div>
										<div class="col-sm-6">
											<button id="editPassword" class="form-control btn btn-password" data-action="password">' . __(
                    'Change password'
                ) . '</button>
											<button id="delete" class="form-control btn btn-delete d-none" data-toggle="modal" data-target="#confirm_delete">' . __(
                    'Delete'
                ) . '</button>
										</div>
									</div>
