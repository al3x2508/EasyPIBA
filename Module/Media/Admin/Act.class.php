...
class Act extends AdminAct
{
            ...
            if(strtolower($path_parts['extension']) === 'webp') {
                ...
                if ($mediaType === 1) {
                    ...
                    if (move_uploaded_file($file['tmp_name'], $target_file)) {
                        $path_parts = pathinfo($target_file);
                        exec("ffmpeg -i \"{$target_file}\" \"{$target_dir}{$path_parts['filename']}." . ((strtolower($path_parts['extension']) == 'mp4')?'ogv':'mp4') . "\" > /dev/null &");
                    ...
                    if (move_uploaded_file($file['tmp_name'], $target_file)) {
                        $path_parts = pathinfo($target_file);
                        exec("ffmpeg -i \"{$target_file}\" \"{$target_dir}{$path_parts['filename']}." . ((strtolower($path_parts['extension']) == 'mp3')?'ogg':'mp3') . "\" > /dev/null &");
                        return array('src' => $target_file, 'media' => $filename, 'type' => $mediaType);
                    } else {
                        $error = __('Error uploading');
                    }
    ...
    private static function uploadFile($file, $moveUploaded = true)
    {
        ...
        if ($file['size'] <= 104857600) {
            ...
            $filename = $uid . Util::getUrlFromString(basename($path_parts['filename'])) . '.' . strtolower($path_parts['extension']);
            $target_dir = _APP_DIR_ . 'uploads/';
            if (!AdminController::getCurrentUser()) {
                $target_dir .= $userId . '/';
                if (!file_exists($target_dir)) {
