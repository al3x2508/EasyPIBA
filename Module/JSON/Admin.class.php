...
class Admin
{
    ...
    public function __construct()
    {
        ...
        if (!Controller::getCurrentUser() && (isset($this->permission) && $adminController->checkPermission($this->permission) == false)) {
    ...
    public function excelExport()
    {
        ...
        foreach ($this->data AS $entity) {
            foreach (get_object_vars($entity) AS $key => $value) {
                if (!is_array($value) && !is_object($value)) {
                    if ((!isset($this->columnsMap) || !is_array($this->columnsMap) || (count($this->columnsMap) == 0)) || arrayKeyExists($key,
                            $this->columnsMap)
                    ) {
                        $data[$key] = $value;
                    ...
                    foreach (get_object_vars($value) AS $key2 => $value2) {
                        if (!is_array($value2) && !is_object($value2)) {
                            $columnName = $key . '.' . $key2;
                            if ((!isset($this->columnsMap) || !is_array($this->columnsMap) || (count($this->columnsMap) == 0)) || arrayKeyExists($columnName,
                                    $this->columnsMap)
                            ) {
        ...
        foreach ($columnNames AS $index => $columnName) {
            if ((!isset($this->columnsMap) || !is_array($this->columnsMap) || (count($this->columnsMap) == 0)) || arrayKeyExists($columnName,
                ...
                $columnName = (is_array($this->columnsMap) && arrayKeyExists($columnName,
                        $this->columnsMap))?$this->columnsMap[$columnName]:$columnName;
        ...
        $objPHPExcel->getActiveSheet()->getPageSetup()->setRowsToRepeatAtTopByStartAndEnd(1,
            1)->setOrientation(PageSetup::ORIENTATION_LANDSCAPE);
